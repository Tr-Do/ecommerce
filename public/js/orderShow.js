...
async function pollPaid(maxSeconds = 5) {
  ...
  while (Date.now() < deadline) {
    const result = await fetch(
      `/downloads/session/${encodeURIComponent(sessionId)}/status`,
    );
...
async function loadFiles() {
  const result = await fetch(
    `/downloads/session/${encodeURIComponent(sessionId)}/files`,
  );
...
function renderLink(files) {
  ...
  for (const file of files) {
    ...
    li.className =
      "d-flex justify-content-center align-items-center mt-3 gap-5";
...
(async () => {
  try {
    ...
    status.textContent =
      "Your files are ready for download. The link(s) will expire in 12 hours!";
  } catch (e) {
